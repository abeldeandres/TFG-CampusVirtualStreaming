...
<ol class="breadcrumb">
	...
	<li class="active">Listado de Recursos</li>
</ol>
...
<div class="row">
	...
	foreach ( $listaRecursos as $recurso ) {
		$ruta = split ( "/", $recurso ['ruta'] );
		$profesor = obtenerUsuario ( $recurso ['autor'] );
		?>
            <tr>
								<?php
		if ($recurso ['autor'] == $_SESSION ['usuario'] ['id_usuario']) {
			?>
								<td><a
...
if ($recurso ['ruta'] == '../archivos/') {
			...
			?><a target="_blank"
								...
								<td>
									<form name="borrar"
										action="gestionar_recursos.php?idTema=<?php echo $id_tema?>"
										method="POST">
